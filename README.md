...
- **主题模式**：支持浅色、深色和跟随 Windows 系统主题，窗口标题栏、设置、菜单与工作台会同步切换。
- **CLI 更新中心**：每天自动读取官方稳定版信息，也可手动检查；识别 npm、pnpm、Yarn 和 Bun 全局安装，确认后可安全停止本地服务、升级、校验版本并按需恢复服务。
...
- **任务交互**：处理问题、待办和子任务活动；右侧面板区分“运行中、已有进展、等待完成通知、已完成、失败、已超时、已取消”等后台任务状态，并显示子 Agent 标识、结果摘要、输出尾部与错误详情。
...
- **能力与兼容性检测**：设置页显示桌面端、系统 CLI 和本地服务版本，并以“已可用 / 当前版本未提供 / 正在检测 / 暂未确认”说明各组能力；可随时重新检测。
...
`0.6.0` 补齐了桌面应用需要长期使用的外观和维护能力，同时继续兼容 Kimi Code CLI `0.30.x`：

- 提供浅色、深色和跟随 Windows 系统的三态主题，偏好持久化到桌面端独立配置；
- 主进程以系统主题为唯一来源，渲染界面、原生标题栏和窗口背景同步变化，避免启动闪烁；
- 每 24 小时自动检查一次 Kimi Code 官方稳定版清单，支持官方灰度发布时间与离线缓存；
- 在设置页、侧栏状态和命令面板中显示非阻塞升级提醒，并提供中文升级确认流程；
- 自动识别 npm、pnpm、Yarn 与 Bun 的全局安装来源，只执行桌面端生成的固定命令，不接受渲染层传入可执行文件、参数、包名或目标版本；
- 一键升级会安全关闭由桌面端启动的本地 CLI 服务，限制并脱敏安装日志，升级后重新发现 CLI、校验目标版本，并仅在升级前服务处于运行状态时自动恢复；
- 无法可靠识别安装来源时不会冒险自动执行，而是提供 `kimi upgrade` 手动命令。

截至 **2026 年 7 月 30 日**，官方稳定版清单为 Kimi Code CLI `0.31.0`；桌面端仍保留 `0.30.x` 兼容模式，新能力会按本地服务实际接口渐进启用。

## 0.5.0 兼容能力基础

`0.5.0` 为后续逐步接入上游新功能建立了统一的能力层，同时继续优先兼容系统中已安装的 Kimi Code CLI `0.30.x`：

- 主进程综合 CLI 版本、本地服务版本、安全接口探测和真实业务请求结果，维护 `supported / unsupported / unknown` 三态能力快照；
- 设置页集中显示版本、兼容模式与会话运行状态、结构化时间线、配置读取、辅助模型、托管账户、提示词配置和非阻塞任务输出等能力组；
- `0.30.x` 缺少的新接口会显示为“当前版本未提供”，不会被当作应用故障，也不会影响现有工作区、任务和对话流程；
- `0.31.x` 及后续 CLI 暴露新接口后，桌面端可通过重新检测和真实请求自动确认能力，不需要在渲染进程中硬编码版本判断；
- 后台任务状态统一由结构化事实投影，明确区分快照进展、等待最终通知和结束结果，为后续更完整的 Agent 管理与任务中心打基础。

本版本尚未为辅助模型、托管账户和自定义 Agent 提供完整图形配置入口；能力层只负责可靠识别和兼容降级，具体产品入口会在后续版本中逐步接入。
